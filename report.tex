...
На тетраэдральном конечном элементе удобно определить $\mathcal{L}$-ко\-ор\-ди\-на\-ты, называемые также барицентрическими координатами~\citep{soloveychick}. Введем нумерацию вершин и ребер, показанную на рисунке \ref{fig:tetrahedron}:
...
Иерархический векторный базис Вебба второго порядка второго типа имеет вид~\citep{mikhajlova}:
\begin{equation*}
	\begin{matrix}
		\displaystyle
		...
		\displaystyle
		\mathbf{w}_{i}^{2,\mathrm{II}} = \nabla ( \mathcal{L}_j \mathcal{L}_k \mathcal{L}_l ) ; \text{~~} i = 21, ..., 24 ;
		\scriptstyle
		\text{~~} j, k, l = 1, ..., 4 ; \text{~~} j < k < l ,\\
		\displaystyle
