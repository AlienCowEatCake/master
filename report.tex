...
\begin{figure}[H]
	\centering
	\subfloat[][]{\includegraphics[trim=0mm 0mm 79.2mm 0mm,clip,scale=1]{research-2/meshes.png}\label{fig:meshes_a}}
	\subfloat[][]{\includegraphics[trim=79.2mm 0mm 0mm 0mm,clip,scale=1]{research-2/meshes.png}\label{fig:meshes_b}}
...
Зафиксируем $\delta_k = 100$~м, $l_k = 600$~м, $m = 3$, $h = 5$~м и будем варьировать коэффициент комплексного растяжения координат $\chi$. Выделим внутри области c условием <<большого бака>> и области, на границе которой задан PML-слой, подобласть $\Omega'$ размером $1000 \times 1000 \times 1000$~м${}^3$, для которой оценим разность в норме $L^2$ между действительными компонентами $\mathbf{E}_y$ векторов решений $\mathbf{E}_y^{\text{бак}}$ и $\mathbf{E}_y^{\text{PML}}$, полученных с применением <<большого бака>> и PML-слоя соответственно. Результаты приведены в таблице \ref{tab:tbl_13}.
\begin{table}[H]
	\caption{варьирование коэффициентов растяжения при $h= 5$~м}
	\label{tab:tbl_13}
	\begin{tabularx}{\textwidth}{|C{0.65}|C{0.65}|C{0.65}|C{0.65}|C{0.65}|C{0.65}|C{1}|C{1}|C{3.1}|}
		\rowcolor[gray]{.9} \hline $\Re(\chi)$ в $\Omega_1$ & $\Im(\chi)$ в $\Omega_1$ & $\Re(\chi)$ в $\Omega_2$ & $\Im(\chi)$ в $\Omega_2$ & $\Re(\chi)$ в $\Omega_3$ & $\Im(\chi)$ в $\Omega_3$ & Время, бак & Время, PML & \raisebox{-0.7em}{$\smash{\displaystyle \frac{\lVert \Re(\mathbf{E}_y^{\text{бак}}) - \Re(\mathbf{E}_y^{\text{PML}})\rVert}{\lVert \Re(\mathbf{E}_y^{\text{бак}})\rVert}}$} \\
		\hline 3 & 0 & 1 & 5 & 3 & 1 & 650 & 592 & 0.106636 \\
		\hline 3 & 1 & 0 & 6 & 2 & 1 & 650 & 599 & 0.092546 \\
		\hline 4 & 0 & 1 & 5 & 3 & 1 & 650 & 731 & 0.0946679 \\
		\hline 4 & 1 & 0 & 6 & 2 & 1 & 650 & 591 & 0.0909998 \\
		\hline
	\end{tabularx}
\end{table}

Также рассмотрим случай, когда петля расположена на расстоянии $h= -5$~м от границы раздела сред воздух--вода. Результаты приведены в таблице \ref{tab:tbl_14}.
\begin{table}[H]
	\caption{варьирование коэффициентов растяжения при $h= -5$~м}
	\label{tab:tbl_14}
	\begin{tabularx}{\textwidth}{|C{0.65}|C{0.65}|C{0.65}|C{0.65}|C{0.65}|C{0.65}|C{1}|C{1}|C{3.1}|}
		\rowcolor[gray]{.9} \hline $\Re(\chi)$ в $\Omega_1$ & $\Im(\chi)$ в $\Omega_1$ & $\Re(\chi)$ в $\Omega_2$ & $\Im(\chi)$ в $\Omega_2$ & $\Re(\chi)$ в $\Omega_3$ & $\Im(\chi)$ в $\Omega_3$ & Время, бак & Время, PML & \raisebox{-0.7em}{$\smash{\displaystyle \frac{\lVert \Re(\mathbf{E}_y^{\text{бак}}) - \Re(\mathbf{E}_y^{\text{PML}})\rVert}{\lVert \Re(\mathbf{E}_y^{\text{бак}})\rVert}}$} \\
		\hline 4 & 0 & 1 & 5 & 3 & 1 & 309 & 344 & 0.0929047 \\
		\hline 4 & 0 & 1 & 6 & 3 & 1 & 309 & 294 & 0.086956 \\
		\hline 4 & 0 & 1 & 6 & 3 & 2 & 309 & 253 & 0.0809429 \\
		\hline 4 & 1 & 1 & 6 & 3 & 2 & 309 & 306 & 0.0657666 \\
		\hline
	\end{tabularx}
\end{table}

\subsubsection{Варьирование толщины PML-слоя}
Зафиксируем $\chi_{\Omega_1} = (4, 1)$, $\chi_{\Omega_2} = (0, 6)$, $\chi_{\Omega_2} = (2, 1)$, $l_k = 600$~м, $m = 3$, $h = 5$~м и будем варьировать толщину PML-слоя $\delta_k$. Результаты приведены в таблице \ref{tab:tbl_15}.
\begin{table}[H]
	\caption{варьирование толщины PML-слоя при $h= 5$~м}
	\label{tab:tbl_15}
	\begin{tabularx}{\textwidth}{|C{0.5}|C{1}|C{1}|C{1.5}|}
		\rowcolor[gray]{.9} \hline $\delta_k$ & Время, бак & Время, PML & $\displaystyle \frac{\lVert \Re(\mathbf{E}_y^{\text{бак}}) - \Re(\mathbf{E}_y^{\text{PML}})\rVert}{\lVert \Re(\mathbf{E}_y^{\text{бак}})\rVert}$ \\
		\hline 80 & 673 & 1289 & 0.119883 \\
		\hline 100 & 650 & 591 & 0.0909998 \\
		\hline 120 & 609 & 1142 & 0.078364 \\
		\hline
	\end{tabularx}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|C{0.5}|C{1}|C{1}|C{1.5}|}
		\rowcolor[gray]{.9} \hline $\delta_k$ & Время, бак & Время, PML & $\displaystyle \frac{\lVert \Re(\mathbf{E}_y^{\text{бак}}) - \Re(\mathbf{E}_y^{\text{PML}})\rVert}{\lVert \Re(\mathbf{E}_y^{\text{бак}})\rVert}$ \\
		\hline 80 & 359 & 297 & 0.12013 \\
		\hline 100 & 309 & 253 & 0.0809429 \\
		\hline 120 & 250 & 859 & 0.0623323 \\
		\hline
	\end{tabularx}
\end{table}
...
\begin{figure}[H]
	\centering
	\subfloat[][]{\includegraphics[scale=0.5]{research-2/field/pml_sca.png}}
	\subfloat[][]{\includegraphics[scale=0.5]{research-2/field/std_sca.png}}
...
\begin{figure}[H]
	\centering
	\subfloat[][]{\includegraphics[scale=0.5]{research-2/field/pml_vec.png}}
	\subfloat[][]{\includegraphics[scale=0.5]{research-2/field/std_vec.png}}
